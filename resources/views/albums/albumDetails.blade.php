...
@section('content')
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">{{ __('Album Details') }}</div>
                    ...
                    <div class="card-body">
                        <table class="table table-bordered">
                            ...
                            <tr>
                                <td>Album Name</td>
                                <td><b>{{ $albumDetail[0]->name }}</b></td>
                            </tr>
                            <tr>
                                <td>Artist</td>
                                <td><b>{{ $albumDetail[0]->artist }}</b></td>
                            </tr>
                            @php
                                $metadata = $albumDetail[0]->metadata;
                        ...
                        <table class="table table-bordered">
                            <thead>
                                <tr><td>Song List</td></tr>
                            </thead>
                            <tbody>
                            @if(!empty($songs) && $songs->count())
                                @foreach($songs as $key => $value)
                                    <tr>
                                        <td><a href="{{ URL::to("songs/{$value->id}") }}">{{ $value->name }}</a></td>
                                    </tr>
                                @endforeach
                            @else
                                <tr>
                                    <td colspan="10">There are no Songs for this Album.</td>
                                </tr>
                            @endif
                            </tbody>
                        </table>
                ...
                <div class="card card-info">
                    <div class="card-block">
                        <form method="post" class="form-inline" action="{{ route('comments.store') }}">
                            @csrf
                            <input type="hidden" name="album_id" value="{{ $albumDetail[0]->id }}" />
